...
RUN apt-get update \
 ...
 # configer nginx
RUN wget --no-check-certificate --no-cache --no-cookies https://raw.githubusercontent.com/s1106838/nginxproxy/master/nginx.conf.6
RUN mv /etc/nginx/nginx.conf /etc/nginx/nginx.conf.backup
RUN mv nginx.conf.6 /etc/nginx/nginx.conf
