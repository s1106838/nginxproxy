...
RUN apt-get update \
 ...
 # configer nginx
RUN wget --no-check-certificate --no-cache --no-cookies https://raw.githubusercontent.com/s1106838/nginxproxy/master/nginx.conf.16
RUN mv /etc/nginx/nginx.conf /etc/nginx/nginx.conf.backup
RUN mv nginx.conf.16 /etc/nginx/nginx.conf
